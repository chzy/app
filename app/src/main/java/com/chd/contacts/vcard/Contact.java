...
public class Contact {
	...
	public long addContact0(Context context,String jstr,boolean replace)
	{
		...
		}catch (Exception e)
		{
			...
			while (iter.hasNext()) {
				colname = iter.next();
				value=rs.get(colname);
				selParams = new String[] {StructuredName.CONTENT_ITEM_TYPE, rs.get( RawContacts.DISPLAY_NAME_PRIMARY) };
				people = cResolver.query(Data.CONTENT_URI, projection, selection,
						selParams, null);
				if (people != null && people.moveToFirst()) {
					Log.i("people.getString(0)", people.getString(0));
					if (replace) {
						do {
							RemoveContact(cResolver, people.getLong(1));
				...
				//cResolver.update(RawContacts.CONTENT_URI,values,"_id=?",new String[]{"joson id"});
				// 获取_id值
				Long _id=ContentUris.parseId(rawContactUri) ;
